// مخزن حالة مشترك للوحة التحكم — يحفظ في localStorage
...
export type AdminConfig = {
  exchangeRate: number; // متوسط (للتوافق)
  buyRate: number;  // سعر الشراء (يشتري المستخدم USD مقابل SYP)
  sellRate: number; // سعر البيع (يبيع المستخدم USD مقابل SYP)
  ...
  whatsapp: string; // رقم واتساب بصيغة دولية بدون +
  telegram: string; // اسم مستخدم تيليجرام بدون @
  supportPhone: string;
  // معلومات الإيداع البنكي
  bankName: string;
  ...
  depositContactPhone: string;
  depositContactWhatsapp: string;
};

export type AdminAccount = { username: string; password: string; createdAt: string };
...
export type StoredTransaction = {
  ...
  userId: string;
  userName: string;
  type: "deposit" | "withdraw" | "transfer" | "investment" | "profit" | "exchange" | "admin_credit" | "admin_debit";
  amount: number;
  currency: "SYP" | "USD";
  date: string;
...
export type StoredPlan = {
  ...
  minAmount: number; // بالدولار
  maxAmount: number; // بالدولار
...
const defaultStore: Store = {
  ...
  transactions: [
    { id: "at1", userId: "AMN-2K8X4P", userName: "بدر الشمري", type: "deposit", amount: 50000, currency: "SYP", date: "اليوم ١٠:٢٠", status: "pending" },
    { id: "at2", userId: "AMN-7HQ2WL", userName: "أحمد الخطيب", type: "withdraw", amount: 12000, currency: "SYP", date: "اليوم ٠٩:٤٥", status: "pending" },
  ...
  plans: [
    { id: "p1", name: "خطة البداية", description: "خطة آمنة للمبتدئين بعوائد يومية مستقرة", profitRate: 0.8, profitType: "daily", duration: 30, durationUnit: "يوم", minAmount: 50, maxAmount: 1000, risk: "low", badge: "آمن", enabled: true },
    { id: "p2", name: "محفظة النمو", description: "استثمار متوازن في أسواق متعددة", profitRate: 12, profitType: "monthly", duration: 3, durationUnit: "شهر", minAmount: 200, maxAmount: 10000, risk: "medium", badge: "متوازن", featured: true, enabled: true },
    { id: "p3", name: "محفظة التقنية العالمية", description: "استثمار عالي النمو في شركات التقنية", profitRate: 1.5, profitType: "daily", duration: 90, durationUnit: "يوم", minAmount: 500, maxAmount: 50000, risk: "high", badge: "عالي النمو", enabled: true },
    { id: "p4", name: "صندوق الصكوك الإسلامية", description: "دخل ثابت حلال بمخاطر منخفضة", profitRate: 7.2, profitType: "monthly", duration: 6, durationUnit: "شهر", minAmount: 100, maxAmount: 20000, risk: "low", badge: "حلال", enabled: true },
  ],
  admins: [
...
export function useStore() {
  ...
  const update = (updater: (s: Store) => Store) => {
...
// عمليات مساعدة
export function adjustUserBalance(store: Store, userId: string, currency: "SYP" | "USD", delta: number, note: string): Store {
  const users = store.users.map((u) =>
    u.id === userId ? { ...u, [currency === "SYP" ? "syp" : "usd"]: (currency === "SYP" ? u.syp : u.usd) + delta } : u
  );
